...
// result storage + metadata on a different index
type BleveResultStorage struct {
	index   bl.Index
	mapping m.IndexMapping
	meta    cloudsearch.MetadataStorage
	favorites cloudsearch.FavoritesStorage
}
...
func NewResultStorage(index bl.Index, meta cloudsearch.MetadataStorage, favorites cloudsearch.FavoritesStorage) cloudsearch.ResultsStorage {
...
func (f *BleveResultStorage) DeleteAllFromAccount(accountId string) ([]string, error) {
	...
	for _, r := range ret {
		_ = f.meta.DeleteThumbnail(r)
	}

	_ = f.meta.ClearWatermarks() // TODO clear _only_ for the given accountId?
...
func (s *BleveResultStorage) Delete(resultId string) error {
	...
	return s.meta.DeleteThumbnail(resultId)
}
...
func (s *BleveResultStorage) Get(resultId string) (*cloudsearch.Result, error) {
	...
	// load fave status
	if res != nil {
		res.Favorited, err = s.favorites.IsFavorite(resultId)
	}
...
func (f *BleveResultStorage) AllFavorited() ([]cloudsearch.Result, error) {
	ids, err := f.favorites.AllFavoritedIds()
	if err != nil {
		return nil, err
	}
